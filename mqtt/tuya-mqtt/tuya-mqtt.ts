...
import { createTable, DataTypes, openDB } from "./lib/database";
...
// Main code function
const main = async () => {
  logger.info("Starting tuya-mqtt");
  await openDB();

  await createTable(KnnectTableTypes.VOLTAGE, {
    id: DataTypes.TEXT,
    voltage: DataTypes.REAL,
  });
  await createTable(KnnectTableTypes.POWER, {
    id: DataTypes.TEXT,
    power: DataTypes.REAL,
});
  await createTable(KnnectTableTypes.CURRENT, {
    id: DataTypes.TEXT,
    current: DataTypes.REAL,
});

  await createTable(KnnectTableTypes.TOTAL, {
    id: DataTypes.TEXT,
    totalEnergy: DataTypes.REAL,
});
