...
import { KnnectTableTypes } from "../lib/dbNames";
...
class TuyaDevice {
        ...
        if (this.dps[key] !== data.dps[key]) {
          switch (key) {
            case "101":
              ...
              await insert(KnnectTableTypes.TOTAL, [data.devId, data.dps[key]]);
              break;

            case "20":
              logger.info(
                "Changed Voltage from " +
                  this.dps[key]?.val +
                  " to " +
                  data.dps[key]
              );
              await insert(KnnectTableTypes.VOLTAGE, [
                data.devId,
                data.dps[key],
              ]);
              break;
            case "19":
              ...
              await insert(KnnectTableTypes.POWER, [data.devId, data.dps[key]]);
              break;
            case "18":
              logger.info(
                "Changed Current from " +
                  this.dps[key]?.val +
                  " to " +
                  data.dps[key]
              );
              await insert(KnnectTableTypes.CURRENT, [
                data.devId,
                data.dps[key],
              ]);
              break;
            default:
              logger.warn("Key mismatched " + key);
              break;
          }
          this.dps[key] = {
            val: data.dps[key],
  ...
  sendTuyaCommand(message, deviceTopic) {
    ...
    const tuyaCommand: any = new Object();
  ...
  // Publish MQTT
  publishMqtt(topic, message, isDebug = false) {
    if (isDebug) {
